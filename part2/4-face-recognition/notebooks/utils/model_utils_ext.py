...
class ModelUtils():
    def __init__(self, model, device, train_loader, test_loader, start_epoch, epochs, 
        ...
        self.grace_epochs_cnt = 25 # num of epochs we can wait for performance improvement
    ...
    def train(self, cur_epoch):
        ...
        for batch_idx, (data, target) in enumerate(pbar):
          ...
          output = self.model(x=data, target=target, embed=False) # predict
    ...
    def test(self, cur_epoch):
        ...
        with torch.no_grad():
          ...
          for batch_idx, (data, target) in enumerate(pbar):
            data, target = data.to(self.device), target.to(self.device)
            output = self.model(x=data, target=None, embed=False) # predict
            loss = self.criterion(output, target) # calculate loss
    ...
    def build(self):
        ...
        for epoch in range(self.start_epoch, self.epochs+1):
          ...
          if((epoch-epoch_max_acc) > self.grace_epochs_cnt):
            print(f'\nExisting training as test accuracy did not improved for last {self.grace_epochs_cnt} epochs')
            break
...
def get_class_based_accuracy(model, device, testloader, classes):
    ...
    class_total = [1 for i in class_total] # this is to avoid divide by zero error in case there are classes without any test samples. 
    with torch.no_grad():
...
def get_accuracy_for_specific_classes(model, device, testloader, class_names, acc_class_names):
    class_ids = [class_names.index(classname) for classname in acc_class_names]
    ...
    with torch.no_grad():
        for data in testloader:
            ...
            for i in range(len(labels)):
                label = labels[i]
                if(label in class_ids): # calculate accuracy only for speified classes
                  idx = class_ids.index(label)
                  class_correct[idx] += c[i].item()
                  class_total[idx] += 1

    for i in range(num_classes):
        print(f'Accuracy of {acc_class_names[i]} : {100*class_correct[i]/class_total[i]:0.2f}%')
    return
